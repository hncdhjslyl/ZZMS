...
public class ItemMakerHandler {
    ...
    public static void ProfessionInfo(final LittleEndianAccessor slea, final MapleClient c) { //so pointless
        try {
            ...
            if (stat.startsWith("9200") || stat.startsWith("9201")) {
                ...
                if ((stat.equalsIgnoreCase("hyper")) || (stat.equalsIgnoreCase("hyper_shaman"))) {
                    int chance = 0;
                    if ((mode == 0) && ((array == 28) || (array == 30) || (array == 32) || (array == 34) || (array == 38) || (array == 40))) {
                        chance = 1;
                    } else if ((mode == 1) && ((array == 28) || (array == 32) || (array == 36) || (array == 38) || (array == 40))) {
                        chance = 1;
                    } else if ((mode == 2) && ((array == 30) || (array == 34) || (array == 40))) {
                        chance = 1;
                    }
                    c.getSession().write(CWvsContext.updateSpecialStat(stat, array, mode, array < 41, chance));
                    return;
                }
                rate = Math.max(0, 100 - ((array + 1) - c.getPlayer().getProfessionLevel(Integer.parseInt(stat))) * 20);
            }
            c.getSession().write(CWvsContext.updateSpecialStat(stat, array, mode, rate));
    ...
    public static final void CraftComplete(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (craftID == 92049000) { //disassembling
            ...
            if (item1.getState(false) == item2.getState(false) && item1.getState(false) > 17) {
            ...
            if (Randomizer.nextInt(100) < (newEquip.getUpgradeSlots() > 0 || potentialChance >= 100 ? potentialChance : (potentialChance / 2))) {
                newEquip.resetPotential_Fuse(theLevl > 5, potentialState, false);
            }

...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            attacker.gainExpMonster(exp, true, highestDamage, pty, Class_Bonus_EXP_PERCENT, Premium_Bonus_EXP_PERCENT, this);
        }
            ...
            default: {
                for (final int i : toSpawn) {
                    ...
                    if (mob.getId() == 9300216) {
                        ...
                        map.broadcastMessage(CField.environmentChange("dojang/end/clear", 13));//was3
                    }
                }
                break;
            }
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        final Runnable applyEffectTask = new Runnable() {
            ...
            public final void run() {
                if (status.getPoisonSchedule() != 0 && isAlive() && (hp - status.getPoisonSchedule()) > 1) {
                    if (from.isShowInfo()) {
                        from.showMessage(10, "執行 => 持續傷害: 傷害[" + status.getPoisonSchedule() + "] 執行時間[" + System.currentTimeMillis() + "]");
                    }
        ...
        final Runnable cancelTask = new Runnable() {
            ...
            public final void run() {
                if (isAlive()) {
                    map.broadcastMessage(MobPacket.cancelMonsterStatus(MapleMonster.this, status), getPosition());
                    if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                        getController().getClient().getSession().write(MobPacket.cancelMonsterStatus(MapleMonster.this, status));
                    }
                    stati.remove(stat);
                }
        ...
        if (from.isShowInfo()) {
            from.showMessage(10, "開始 => 給予怪物狀態: 持續時間[" + aniTime + "] 狀態效果[" + status.getStati().name() + "] 開始時間[" + System.currentTimeMillis() + "]");
        }
        // 持續傷害的Buff
        if (status.getStati() == MonsterStatus.POISON || status.getStati() == MonsterStatus.NINJA_AMBUSH) {
            ...
            if (from.isShowInfo()) {
                from.showMessage(10, "開始 => 持續傷害: 重複執行時間[" + reTime + "ms] 目前時間[" + System.currentTimeMillis() + "]");
            }
        }
    }
        ...
        final Runnable cancelTask = new Runnable() {
            ...
            public void run() {
                if (isAlive()) {
                    ...
                    if (getController() != null && !getController().isMapObjectVisible(MapleMonster.this)) {
                        getController().getClient().getSession().write(MobPacket.cancelMonsterStatus(MapleMonster.this, mse));
                    }

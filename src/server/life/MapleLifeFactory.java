...
public class MapleLifeFactory {
    ...
    private static final Map<Integer, String> npcNames = new TreeMap<>((v1, v2) -> v1.compareTo(v2));
    private static final Map<Integer, MapleMonsterStats> monsterStats = new TreeMap<>((v1, v2) -> v1.compareTo(v2));
    private static final Map<Integer, Integer> NPCLoc = new TreeMap<>((v1, v2) -> v1.compareTo(v2));
    private static final Map<Integer, List<Integer>> questCount = new TreeMap<>((v1, v2) -> v1.compareTo(v2));
    ...
    public static void loadQuestCounts(boolean reload) {
        ...
        for (MapleDataDirectoryEntry mapz : data2.getRoot().getSubdirectories()) {
            if (mapz.getName().equals("QuestCountGroup")) {
                for (MapleDataFileEntry entry : mapz.getFiles()) {
                    final int id = Integer.parseInt(entry.getName().substring(0, entry.getName().length() - 4));
                    MapleData dat = data2.getData("QuestCountGroup/" + entry.getName());
                    if (dat != null && dat.getChildByPath("info") != null) {
                        List<Integer> z = new ArrayList<>();
                        for (MapleData da : dat.getChildByPath("info")) {
                            z.add(MapleDataTool.getInt(da, 0));
    ...
    public static MapleMonsterStats getMonsterStats(int mid) {
        ...
        if (stats == null) {
            ...
            if (monsterData == null) {
                monsterData = data2.getData(StringUtil.getLeftPaddedStr(Integer.toString(mid) + ".img", '0', 11));
                if (monsterData == null) {
                    return null;
                }
                secondData = true;
            ...
            stats.setHp((int) (GameConstants.getPartyPlayHP(mid) > 0 ? GameConstants.getPartyPlayHP(mid) : MapleDataTool.getLongConvert("maxHP", monsterInfoData)));
            ...
            stats.setWP((short) MapleDataTool.getIntConvert("wp", monsterInfoData, 0));

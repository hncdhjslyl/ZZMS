...
public class MapleQuest implements Serializable {
    ...
    private static final Map<Integer, MapleQuest> quests = new TreeMap<>((v1, v2) -> v1.compareTo(v2));
    private static final Map<Integer, MapleQuest> farmQuests = new TreeMap<>((v1, v2) -> v1.compareTo(v2));
    protected int id;
    ...
    public boolean canStart(MapleCharacter c, Integer npcid) {
        ...
        for (MapleQuestRequirement r : startReqs) {
            ...
            if (!r.check(c, npcid)) {
                if (r.getType() == MapleQuestRequirementType.job || r.getType() == MapleQuestRequirementType.job_TW) {
                    if (jobs) {
                        jobs = !jobs;
                        continue;
                    }
    ...
    private boolean checkNPCOnMap(MapleCharacter player, int npcid) {
        //mir = 1013000
        return (MapleJob.is龍魔導士(player.getJob()) && npcid == 1013000) || MapleJob.is天使破壞者(player.getJob()) && npcid == 3000018 || npcid == 9000040 || npcid == 9000066 || (player.getMap() != null && player.getMap().containsNPC(npcid));
    }
    ...
    public static enum MedalQuest {
        ...
        新手探險家(29005, 29015, 15, new int[]{100000000, 100020400, 100040000, 101000000, 101020300, 101040300, 102000000, 102020500, 102030400, 102040200, 103000000, 103020200, 103030400, 103040000, 104000000, 104020000, 106020100, 120000000, 120020400, 120030000}),
